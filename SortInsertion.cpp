...
myArray readFile(char *path){
  ...
  myArray arr;
  initArray(arr);
  fscanf(fp,"%d",&arr.num); arr.a = new int[arr.num];
...
int partition(myArray arr, int start, int end){
  ...
  while(true){
    while(left<=right && arr.a[left]<=arr.a[pivot]) left++;
    while(right>=left && arr.a[right]>=arr.a[pivot]) right--;
    if(left>right) break;
    swap(arr,left,right);
    left++;
    right--;
